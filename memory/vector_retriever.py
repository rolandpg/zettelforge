...
class VectorRetriever:
    ...
    def retrieve(
        ...
        # Get all notes (optionally filtered by domain)
        if domain:
            candidates = self.store.get_notes_by_domain(domain)
        else:
            candidates = list(self.store.iterate_notes())
        ...
        if include_links and results:
            results = self._expand_via_links(results, query_vector, k * 2)
    ...
    def _expand_via_links(
        ...
        all_notes = set(n.id for n in initial_results)
        ...
        for note in initial_results:
            for linked_id in note.links.related:
                ...
                if linked_note and linked_note.id not in all_notes:
                    expanded.append(linked_note)
                    all_notes.add(linked_note.id)
    ...
    def get_memory_context(
        ...
        for i, note in enumerate(notes, 1):
            ...
            recency = note.created_at[:10]  # YYYY-MM-DD
