...
class EvolutionDecider:
    ...
    Phase 4.5 Epistemic Tiering rules:
    - Tier A (authoritative): Human/Tool facts. Can supersede B/C, cannot be superseded by B/C
    - Tier B (operational): Agent direct reports. Can supersede C, cannot supersede A
...
- REJECT: Lower-tier note cannot supersede higher-tier note (log this)

Respond ONLY with valid JSON:
{"decision": "NO_CHANGE|UPDATE_CONTEXT|UPDATE_TAGS|UPDATE_BOTH|SUPERSEDE|REJECT", "reason": "brief explanation"}"""
    ...
    def assess(
        ...
        """
        new_tier = new_note.metadata.tier
        existing_tier = existing_note.metadata.tier

        # Tier enforcement checks (overrides LLM)
        if new_tier == 'C':
            # Tier C never triggers supersession - only UPDATE
            decision = 'NO_CHANGE'
            reason = 'Tier C notes never trigger supersession or context updates'
            if log_reasoning:
                self._log_reject(new_note, existing_note, decision, reason)
            return decision, reason

        if new_tier == 'B' and existing_tier == 'A':
            # Tier B cannot supersede Tier A
            decision = 'REJECT'
            reason = f'Tier B note cannot supersede Tier A note ({existing_note.id})'
            if log_reasoning:
                self._log_reject(new_note, existing_note, decision, reason)
            return decision, reason

        if new_tier == 'A' and existing_tier == 'B':
            # Tier A can supersede Tier B
            decision = 'SUPERSEDE'
            reason = f'Tier A note supersedes Tier B note ({existing_note.id})'
            if log_reasoning:
                get_reasoning_logger().log_evolution(
                    note_id=new_note.id,
                    decision=decision,
                    reason=reason,
                    tier=new_tier,
                    superseded_note_id=existing_note.id,
                    extra={'existing_tier': existing_tier, 'new_tier': new_tier}
                )
            return decision, reason

        if new_tier == existing_tier == 'A':
            # Both Tier A - can supersede but rarely needed
            decision = 'SUPERSEDE'
            reason = f'New Tier A supersedes existing Tier A ({existing_note.id})'
            if log_reasoning:
                get_reasoning_logger().log_evolution(
                    note_id=new_note.id,
                    decision=decision,
        ...
        # For all other cases, use LLM assessment
        try:
            import ollama

            prompt = f"""You are a memory evolution assessor. Given a new note and a candidate existing note,
determine if the existing note should be updated based on the new information.
...
EXISTING NOTE ({existing_note.id}) ({existing_tier}):
...
Assess if the existing note should evolve, respecting epistemic tier rules."""
            ...
            # Log reasoning for evolution decisions
            if decision in ['SUPERSEDE', 'UPDATE_CONTEXT', 'UPDATE_TAGS', 'UPDATE_BOTH'] and log_reasoning:
                get_reasoning_logger().log_evolution(
                    note_id=new_note.id,
                    decision=decision,
                    reason=reason,
                    tier=new_tier,
                    superseded_note_id=existing_note.id if decision == 'SUPERSEDE' else None,
                    extra={'existing_tier': existing_tier, 'new_tier': new_tier}
                )
    ...
    def _log_reject(
        ...
        """Log a REJECT decision when lower-tier cannot supersede higher-tier."""
        get_reasoning_logger().log_evolution(
            note_id=new_note.id,
            decision=decision,
            reason=reason,
            ...
            superseded_note_id=existing_note.id,
            extra={'existing_tier': existing_note.metadata.tier, 'new_tier': new_note.metadata.tier}
        )
...
Response: """
    ...
    def run_evolution_cycle(self, new_note: MemoryNote) -> Dict:
        ...
        # Cap evolution candidates to prevent runaway LLM calls
        MAX_EVOLUTION_CANDIDATES = 10
        capped_related_ids = set(list(related_ids)[:MAX_EVOLUTION_CANDIDATES])
        skipped = len(related_ids) - len(capped_related_ids)
        if skipped > 0:
            print(f"  Note: capping evolution to {MAX_EVOLUTION_CANDIDATES} candidates ({skipped} skipped)")

        if not capped_related_ids:
            print("  No related notes found, skipping evolution")
            return {'evolved': [], 'assessed': 0}

        # Parallel assessment of candidates, sequential evolution
        evolved = []
        assessed_count = 0
        
        # Collect valid candidates
        candidates = []
        for note_id in capped_related_ids:
        ...
        assessment_results = []
        if candidates:
            max_workers = min(len(candidates), 10)  # Cap at 10 parallel workers
        ...
        for note, decision, reason in assessment_results:
            ...
            if decision == 'REJECT':
                print(f"    REJECTED: Tier constraint prevents evolution")
                evolved.append({
                    'note_id': note.id,
                    'decision': decision,
                    'reason': reason
        ...
        return {'evolved': evolved, 'assessed': assessed_count}
