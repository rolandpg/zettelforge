...
| Key | Type | Default | Env Override | Description |
|:----|:-----|:--------|:-------------|:------------|
| `governance.pii.enabled` | `bool` | `False` | `ZETTELFORGE_PII_ENABLED` | Enable Microsoft Presidio PII detection during `remember()`. Soft dependency — requires `pip install zettelforge[pii]` to activate. With `enabled=true` but the SDK missing, the validator constructs successfully and only raises `ImportError` on first detection call. |
