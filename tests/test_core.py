...
class TestVectorRecall:
    ...
    def test_vector_recall_returns_notes(self, temp_memory):
        ...
        mm = MemoryManager(jsonl_path=f"{temp_memory}/notes.jsonl")
        mm.remember(
            "APT28 is a Russian threat actor targeting government entities", domain="security_ops"
        )
        mm.remember(
            "APT29 is also Russian, targeting diplomatic organizations", domain="security_ops"
        )
...
class TestLanceDBIntegration:
    ...
    def test_lancedb_tables_created(self, temp_memory):
        ...
        mm = MemoryManager(
            jsonl_path=f"{temp_memory}/notes.jsonl", lance_path=f"{temp_memory}/vectordb"
        )
